...
xdmFormat::ReadResult XmfReader::readItem( const xdm::FileSystemPath& path ) {
  ...
  // Read the document.
  xdm::RefPtr< impl::XmlDocumentManager > doc(
    new impl::XmlDocumentManager( readDocument( path ) ) );
  ...
  xdm::RefPtr< impl::SharedNodeVector > timestepNodes( new impl::SharedNodeVector );
  if ( temporalCollectionQuery.size() > 0 ) {
    ...
    if ( gridQuery.size() != 0 ) {
      ...
      timestepNodes->push_back( gridQuery.node( 0 ) );
    }
  }

  // Build the XDM data tree.
  impl::TreeBuilder build( doc, timestepNodes );
  result = build.buildTree();
